...
describe("scripts/test-docker-all scheduler", () => {
  ...
  it("validates complete candidate fields against HEAD and tarball bytes", () => {
    const fixture = candidateFixture();
    const plan = {
      needs: { package: true, prepublishPluginRegistry: false },
  ...
  it("preserves legacy release package and registry inputs", () => {
    ...
    env.OPENCLAW_PREPUBLISH_PLUGIN_REGISTRY_DIR = path.relative(
      process.cwd(),
      env.OPENCLAW_PREPUBLISH_PLUGIN_REGISTRY_DIR,
    );
    ...
    expect(() =>
      validateDockerCandidateEnvironment(
        env,
        {
          needs: { package: true, prepublishPluginRegistry: true },
          requiredPrepublishPluginPackages: ["@openclaw/discord"],
        },
        fixture.root,
      ),
    ).not.toThrow();
    expect(env.OPENCLAW_CURRENT_PACKAGE_TGZ).toBe(fixture.packagePath);
    expect(env.OPENCLAW_PREPUBLISH_PLUGIN_REGISTRY_DIR).toBe(path.join(fixture.root, "registry"));
  });
  ...
  it("does not inspect package files for package-free plans", () => {
    ...
    expect(() =>
      validateDockerCandidateEnvironment(
        ...
        {
          needs: { package: false, prepublishPluginRegistry: false },
          requiredPrepublishPluginPackages: [],
        },
        fixture.root,
      ),
  ...
  it("validates complete registry tuples as plan-specific subsets", () => {
    ...
    ).toThrow("missing, extra, or non-file");
  });
