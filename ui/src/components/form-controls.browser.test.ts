...
describeBrowserLayout("app chrome interaction styles", () => {
      ...
      await page.setContent(`
        ...
        <html>
          ...
          <body>
            ...
            <label class="session-menu__emoji-field">Emoji <input value="owl" /></label>
            <button class="session-menu__remove-icon">Remove icon</button>
  ...
  it("scales mobile sidebar variants while preserving the coarse-pointer input floor", async () => {
    ...
    try {
      ...
      expect(scaled.navItem).toBeCloseTo(13 * 1.4, 1);
    } finally {
      await page.close().catch(() => {});
    }
  });
